    ...
    <>
      ...
      <div className="pt-3 w-[100%] flex flex-col gap-2 mobile:w-[470px]">
        <div className="flex items-center justify-between px-1">
          <Link to={`/userprofile/${post.name}`} className="flex items-center gap-2">
            <div className="flex justify-center items-center w-[46px] h-[46px] rounded-full bg-gradient-to-r from-yellow-500 via-red-500 to-purple-500">
              <img src={post.profilePic} className="w-[41px] h-[41px] border-white border-2 rounded-full" alt="" />
            </div>
        ...
        <div className="flex px-2 justify-between">
          <div className="flex gap-2">
            ...
            <FaRegComment size={size} onClick={() => setComment(true)} />
          </div>
          <LuSend size={size} />
        </div>
        <div className="ps-2">
          <p>{numberOfLikes} likes</p>
          <h3><span className="font-bold">{post.name}</span> {post.des}</h3>
          <p className="text-gray-400" onClick={() => setComment(true)}>Comments</p>
        </div>
      </div>
    </>
